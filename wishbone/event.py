...
class Event(object):
        ...
        if self.data["bulk"]:
            if isinstance(event, Event):
                if len(self.data["data"]) == self.bulk_size:
                    raise BulkFull(
                        "The bulk event already contains '%s' events."
                        % (self.bulk_size)
                    )
    ...
    def clone(self):
        ...
        if "uuid_previous" in e.data:
            e.data["uuid_previous"].append(e.data["uuid"])
        else:
            e.data["uuid_previous"] = [e.data["uuid"]]
    ...
    def renderKwargs(self, template_kwargs):
        ...
        self.kwargs = EasyDict(recurse(template_kwargs))
        ...
        try:
            ...
            for item in [
                ...
                if item[1] is not None:
                    assert isinstance(
                        data[item[0]], item[1]
                    ), "%s type '%s' is not valid." % (item[0], item[1])
        except AssertionError as err:
            raise InvalidData(
                "The incoming data could not be used to construct an event.  Reason: '%s'."
                % err
            )
        else:
            self.data = Cut(data)
