...
class ComponentManager:
    ...
    COMPONENT_TYPES = ["protocol", "module", "function"]
    ...
    def getComponent(self, namespace, component_type, category, name):
        ...
        m = None
        for module in pkg_resources.iter_entry_points(
            "%s.%s.%s" % (namespace, component_type, category)
        ):
            if module.name == name:
                m = module.load()
                break
        ...
        if m is None:
            raise NoSuchComponent(
                "Component %s.%s.%s.%s cannot be found."
                % (namespace, component_type, category, name)
            )
        else:
            if (
                issubclass(m, Actor)
                or issubclass(m, TemplateFunction)
                or issubclass(m, ModuleFunction)
                or issubclass(m, Decode)
                or issubclass(m, Encode)
            ):
                return m
            else:
                raise InvalidComponent(
                    "'%s.%s.%s.%s' is not a valid wishbone component."
                    % (namespace, component_type, category, name)
                )
    ...
    def getComponentList(self):
        ...
        for namespace in self.namespace:
            for category in sorted(self.protocol_categories):
                ...
                for item in [
                    m.name for m in pkg_resources.iter_entry_points(group=prefix)
                ]:
    ...
    def getComponentDoc(self, namespace, component_type, category, name):
        ...
        if doc is None:
            raise InvalidComponent(
                "Component '%s' does not seem to have the expected docstring format."
                % ()
            )
        else:
            return doc
    ...
    def validateComponentName(self, name):
        ...
        if len(name.split(".")) != 4:
            raise InvalidComponent(
                "Component name '%s' should consist out of 4 parts." % (name)
            )
        if name.split(".")[1] not in self.COMPONENT_TYPES:
            raise InvalidComponent(
                "Component name '%s' has an invalid component type name." % (name)
            )
