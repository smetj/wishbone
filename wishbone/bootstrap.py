...
class BootStrap:
    ...
    def __init__(
        self, description="Pragmatic event processing servers.", include_groups=[]
    ):
        ...
        start = subparsers.add_parser(
            "start", description="Bootsraps a Wishbone instance."
        )
        start.add_argument(
            "--config",
            type=str,
            dest="config",
            default="wishbone.cfg",
            help="The Wishbone bootstrap file to load.",
        )
        start.add_argument(
            "--frequency",
            type=int,
            dest="frequency",
            default=10,
            help="The metric frequency.",
        )
        start.add_argument(
            "--graph",
            action="store_true",
            dest="graph",
            help="When enabled starts a webserver on 8088 showing a graph of connected modules and queues.",
        )
        start.add_argument(
            "--graph-include-sys",
            action="store_true",
            dest="graph_include_sys",
            help="When enabled includes logs and metrics related queues modules and queues to graph layout.",
        )
        start.add_argument(
            "--identification",
            type=str,
            dest="identification",
            default="wishbone",
            help="An identifier string for generated logs.",
        )
        start.add_argument(
            "--instances",
            type=int,
            dest="instances",
            default=1,
            help="The number of parallel Wishbone instances to bootstrap.",
        )
        start.add_argument(
            "--loglevel",
            type=int,
            dest="log_level",
            default=6,
            help="The maximum loglevel.",
        )
        start.add_argument(
            "--fork",
            action="store_true",
            default=False,
            help="When defined forks Wishbone to background and INFO logs are written to syslog/journald.",
        )
        start.add_argument(
            "--nocolor",
            action="store_true",
            help="When defined does not print colored output to stdout.",
        )
        start.add_argument(
            "--pid",
            type=str,
            dest="pid",
            default="%s/wishbone.pid" % (os.getcwd()),
            help="The pidfile to use.",
        )
        start.add_argument(
            "--profile",
            action="store_true",
            help="When enabled profiles the process and dumps a Chrome developer tools profile file in the current directory.",
        )
        start.add_argument(
            "--queue-size",
            type=int,
            dest="queue_size",
            default=100,
            help="The queue size to use.",
        )

        stop = subparsers.add_parser(
            "stop", description="Tries to gracefully stop the Wishbone instance."
        )
        stop.add_argument(
            "--pid",
            type=str,
            dest="pid",
            default="wishbone.pid",
            help="The pidfile to use.",
        )
        ...
        list.add_argument(
            ...
            type=str,
            dest="namespace",
            default="wishbone, wishbone_contrib, wishbone_external",
            help="The component namespace to query.",
        )

        show = subparsers.add_parser(
            "show", description="Shows information about a component."
        )
        show_group = show.add_mutually_exclusive_group(required=True)
        show_group.add_argument(
            "--docs", type=str, help="Shows the documentation of the component."
        )
        show_group.add_argument(
            "--code", type=str, help="Shows the code of the refered component."
        )
...
class Dispatch:
    ...
    def generateHeader(self):
        ...
        with open("%s/data/banner.tmpl" % (os.path.dirname(__file__))) as f:
            template = "".join(f.readlines()).format(
                version=get_distribution("wishbone").version
            )
            ...
            with DaemonContext(
                ...
                if self.instances == 1:
                    ...
                    for instance in range(self.instances):
                        self.routers.append(
                            gipc.start_process(
                                self.initializeOneRouter, args=(config,), daemon=True
                            )
                        )
    ...
    def list(self):
        ...
        print(
            (
                ComponentManager(
                    namespace=[n.strip() for n in self.namespace.split(",")]
                ).getComponentTable()
            )
        )
    ...
    def show(self):
        ...
        if self.docs is not None:
            ...
            try:
                title = component_manager.getComponentTitle(
                    namespace, component_type, category, name
                )
                version = component_manager.getComponentVersion(
                    namespace, component_type, category, name
                )
                ...
                print(
                    (
                        component_manager.getComponentDoc(
                            namespace, component_type, category, name
                        )
                    )
                )
        ...
        if self.code is not None:
            ...
            print(
                highlight(
                    code, get_lexer_by_name("python"), terminal.TerminalFormatter()
                )
            )
    ...
    def start(self):
        ...
        self.initializeManyRouters(
            config=config, number=self.instances, background=self.fork
        )
